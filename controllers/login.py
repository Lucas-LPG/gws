...
def add_users():
    ...
    if request.method == 'POST':
        ...
        if user in users or user == 'lucas':
            return render_template("errors/error.html", error_message="Esse nome de usuário já existe!")
        else:
            users[user] = password
...
def list_users():
    global users
    return render_template("users/users.html", users=users, user=session.get('user'))
...
def home():
    ...
    if session.get('user'):
        ...
        if (user in users and users[user] == password) or (user in admin and admin[user] == password):
            ...
            return render_template("errors/error.html", error_message="Esse usuário não existe!")
    else:
        return redirect("/")
